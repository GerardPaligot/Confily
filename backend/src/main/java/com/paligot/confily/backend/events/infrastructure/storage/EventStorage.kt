package com.paligot.confily.backend.events.infrastructure.storage
...
class EventStorage(
    ...
    suspend fun getEventFile(eventId: String, updateAt: Long): ExportEvent? = storage
        .download("$eventId/event/exports/$updateAt.json")
        ?.let { Json.decodeFromString(it.decodeToString()) }

    suspend fun uploadEventFile(eventId: String, updateAt: Long, event: ExportEvent): Upload = storage
        .upload(
    ...
    suspend fun getPlanningFile(eventId: String, updateAt: Long): AgendaV4? = storage
        .download("$eventId/planning/exports/$updateAt.json")
        ?.let { Json.decodeFromString(it.decodeToString()) }

    suspend fun uploadPlanningFile(eventId: String, updateAt: Long, planning: AgendaV4): Upload = storage
        .upload(
    ...
    suspend fun getPartnersFile(eventId: String, updateAt: Long): PartnersActivities? = storage
        .download("$eventId/partners/exports/$updateAt.json")
        ?.let { Json.decodeFromString(it.decodeToString()) }
